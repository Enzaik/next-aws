...
function ProjectRow({ hit }) {
  return (
    <tr>
      <td className="px-6 py-3 max-w-0 w-full whitespace-no-wrap text-sm leading-5 font-medium text-gray-900">
        <div className="flex items-center space-x-3 lg:pl-2">
          <div className={`flex-shrink-0 w-2.5 h-2.5 rounded-full ${hit.color}`}></div>
          <a href="#" className="truncate hover:text-gray-600">
            <span>
              {hit.text} <span className="text-gray-500 font-normal">{hit.helpText}</span>{' '}
            </span>
          </a>
        </div>
      </td>
